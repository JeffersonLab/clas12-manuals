...
The Hall B M{\o}ller system measures the polarization of the electron beam delivered to Hall B, and this document details its operating procedures.  The user interface for shift workers is shown in Fig.~\ref{fig:unconfig} and provides direct access to all controls and feedback that the normal operator should need, and its operating procedures are described in Section~\ref{sec:user}.

Configuring for M{\o}ller runs requires acceptable, stable beam already delivered to the {\bf tagger dump/yoke} as in Table~\ref{fig:beamline}.  Consult the run wiki's documentation and beamline manual for general procedures on establishing beam.

\begin{table}[htbp]\centering
    \begin{tabular}{ll}\toprule[1.5pt]
        Beam Destination & Tagger Dump / Yoke \\
        2C21/2C24 Harps Scans & Ok \\ 
        Beam Current & \ibeam \\
        Orbit Locks & OFF (only for M{\o}ller)\\
        \bottomrule[1.5pt]
    \end{tabular}
    \caption{General beamline setup required for M{\o}ller runs.}
\end{table}

\begin{figure}[htbp]\centering
    \includegraphics[width=10cm]{pics/unconfig}
    \caption{\label{fig:beamline}The user interface for shift workers for operating a M{\o}ller run is divided into {\em Status}, {\em Configuration}, {\em Data Acquisition}, {\em Monitoring}, and {\em Logbook} sections.\label{fig:unconfig}}
\end{figure}
